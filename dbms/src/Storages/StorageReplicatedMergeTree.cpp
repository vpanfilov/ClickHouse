...
bool StorageReplicatedMergeTree::tryExecutePartMutation(const StorageReplicatedMergeTree::LogEntry & entry)
{
    ...
    MutationCommands commands = queue.getMutationCommands(source_part, new_part_info.version);
        ...
        try
        {
            ...
            else
            {
                ...
                if (max_future_part_size > 0)
                {
                    ...
                    if (merger.selectPartsToMerge(future_merged_part, false, max_future_part_size, merge_pred))
                    {
                    ...
                    else
                    {
                        ...
                        for (const auto & part : data_parts)
                        {
                            ...
                            if (!merge_pred.canMutatePart(part, desired_mutation_version))
                                continue;
